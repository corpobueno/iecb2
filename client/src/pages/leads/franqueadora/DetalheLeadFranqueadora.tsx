...
export const DetalheLeadFranqueadora: React.FC<{ id: number }> = ({ id }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [lead, setLead] = useState<ILeadsFranqueadora>();

  useEffect(() => {
    if (!id) return;

    setIsLoading(true);
    LeadsFranqueadoraService.getLead(Number(id))
      .then((result) => {
        setIsLoading(false);
